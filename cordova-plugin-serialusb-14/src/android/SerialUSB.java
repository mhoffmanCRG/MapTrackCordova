/**
 * cordova-plugin-serialusb - SerialUSB.java
...
/**
 * Cordova plugin for Android Serial USB communication
 * @author Xavier Seignard <xavier.seignard@gmail.com>
 * @author Dario Cavada <dario.cavada.lab@gmail.com>
 * @author Milоs Petrasinovic <mpetrasinovic@pr-dc.com>
 */
public class SerialUSB extends CordovaPlugin {
	// logging tag
	private final String TAG = SerialUSB.class.getSimpleName();
	// actions definitions
	...
	private final SerialInputOutputManager.Listener mListener =
			new SerialInputOutputManager.Listener() {
				...
				public void onNewData(final byte[] data) {
					SerialUSB.this.updateReceivedData(data);
				}
			};
    ...
    cordova.getThreadPool().execute(new Runnable() {
        public void run() {
            ...
            if (opts.has("vid") && opts.has("pid")) {
                ...
                Object o_vid = opts.opt("vid");
                Object o_pid = opts.opt("pid");
                ...
                if (driver.equals("FtdiSerialDriver"))       customTable.addProduct(vid, pid, FtdiSerialDriver.class);
                else if (driver.equals("CdcAcmSerialDriver")) customTable.addProduct(vid, pid, CdcAcmSerialDriver.class);
                else if (driver.equals("Cp21xxSerialDriver")) customTable.addProduct(vid, pid, Cp21xxSerialDriver.class);
                else if (driver.equals("ProlificSerialDriver")) customTable.addProduct(vid, pid, ProlificSerialDriver.class);
                else if (driver.equals("Ch34xSerialDriver"))  customTable.addProduct(vid, pid, Ch34xSerialDriver.class);
                else { Log.d(TAG, "Unknown driver!"); callbackContext.error("Unknown driver!"); return; }
            ...
            if (!availableDrivers.isEmpty()) {
                ...
                // ---- Patched PendingIntent: explicit + proper flags ----
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                    piFlags = PendingIntent.FLAG_MUTABLE;        // API 31+
                } else if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    piFlags = PendingIntent.FLAG_IMMUTABLE;      // API 23–30
                } else {
                    piFlags = 0;                                  // API <23
                }
                PendingIntent pendingIntent = PendingIntent.getBroadcast(
                        cordova.getActivity(), 0, permissionIntent, piFlags);

                // ---- Register receiver (API 33+ via 3-arg overload) ----
